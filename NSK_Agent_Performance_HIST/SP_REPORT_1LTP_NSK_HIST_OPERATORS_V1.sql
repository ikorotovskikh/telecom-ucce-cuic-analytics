...
/****** Object:  StoredProcedure [dbo].[SP_REPORT_1LTP_NSK_HIST_OPERATORS_V1]    Script Date: 16.07.2026 22:57:56 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[SP_REPORT_1LTP_NSK_HIST_OPERATORS_V1]
      @dtBegin DATETIME = '2024-08-01 00:00:00'
	, @dtEnd DATETIME = '2024-08-02 00:00:00'
	 ,@SGL VARCHAR(MAX) = '(5391,5392)'
    , @AGL VARCHAR(MAX) = '(6395)'
...
DECLARE @sgListNoInput table (id int)
  INSERT INTO @sgListNoInput (id)
   SELECT DISTINCT SkillGroupSkillTargetID from t_Agent_Skill_Group_Interval asgi
   LEFT JOIN t_Skill_Group sg on asgi.SkillGroupSkillTargetID  = sg.SkillTargetID
   WHERE  DateTime >= @dtBeginPeriod AND DateTime < @dtEndPeriod AND (asgi.SkillTargetID IN (SELECT id FROM @agList))
   AND sg.EnterpriseName NOT LIKE '%Cisco_Voice.default%'
   
DECLARE @agListNoInput table (id int)
  INSERT INTO @agListNoInput (id)
   SELECT DISTINCT SkillTargetID from t_Agent_Skill_Group_Interval asgi
   WHERE  DateTime >= @dtBeginPeriod AND DateTime < @dtEndPeriod AND (SkillGroupSkillTargetID IN (SELECT id FROM @sgList))
...
SELECT
			...
			,rT.[ReasonCode]
			,SecondsByEventInPeriod = SUM(DATEDIFF(SECOND, IIF(@dtBeginPeriod >= EventDtFrom,@dtBeginPeriod,EventDtFrom) , IIF(@dtEndPeriod <= EventDtTo, @dtEndPeriod, EventDtTo)))
			FROM
			(
				SELECT
				 SkillTargetID
				...
				WHERE
					...
					(
					 ...
					 OR
					 (Event = 1 AND DateTime >= @dtBeginPeriod
					  AND NOT EXISTS (SELECT Event FROM t_Agent_Event_Detail WHERE SkillTargetID = aed.SkillTargetID AND Event = 2 AND DateTime > aed.DateTime)  
					 ) 
					)

					AND (SkillTargetID IN (SELECT id FROM @agList)  OR (0 IN (SELECT id FROM @agList) AND SkillTargetID IN (SELECT id FROM @agListNoInput)))
			)rT
			GROUP BY  rT.SkillTargetID, rT.[Event], rT.[ReasonCode]		
		) t
		WHERE 
			SkillTargetID IN (SELECT id FROM @agList)  OR (0 IN (SELECT id FROM @agList) AND SkillTargetID IN (SELECT id FROM @agListNoInput))
		...
		FROM 
          ( 
            ...
            WHERE DateTime >= @dtBeginPeriod AND DateTime < DATEADD(MINUTE, 480, @dtEndPeriod) AND
			      ...
			      (0 IN (SELECT id FROM @sgList)) AND SkillGroupSkillTargetID IN (SELECT id FROM @sgListNoInput)))
                    AND ANI IN 
                               (
                                 SELECT ANI FROM t_Termination_Call_Detail 
		                         WHERE DateTime >= @dtBeginPeriod AND DateTime < DATEADD(MINUTE, 480, @dtEndPeriod) AND
								       ((AgentSkillTargetID IS NOT NULL AND 
		...
		WHERE
			AgentSkillTargetID IN (SELECT id FROM @agList)  OR (0 IN (SELECT id FROM @agList) AND AgentSkillTargetID IN (SELECT id FROM @agListNoInput))
		...
		SELECT 
			...
			,0 AS OtherBreakTime
			,0 AS rc_num
		FROM t_Agent_Skill_Group_Interval
		WHERE
			(SkillTargetID IN (SELECT id FROM @agList)  OR (0 IN (SELECT id FROM @agList) AND SkillTargetID IN (SELECT id FROM @agListNoInput)))
			AND DateTime >= @dtBeginPeriod
			AND DateTime < @dtEndPeriod
  ...
  SELECT
	...
	FROM (
		...
		AND ((SkillTargetID IN (SELECT id FROM @agList))  OR (0 IN (SELECT id FROM @agList) AND SkillGroupSkillTargetID IN  (SELECT id FROM @sgList)))
...
WHERE SkillID IS NULL OR SkillID IN (SELECT id FROM @sgList) OR (0 IN (SELECT id FROM @sgList) AND SkillID IN (SELECT id FROM @sgListNoInput))

ORDER BY AgentID, order_id, SkillName
